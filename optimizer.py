...
def optimize_shifts(
            ...
            for d in range(num_days):
                for sh_idx, label in enumerate(shift_labels):
                    if label not in allowed_shifts:
                        ...
                        objective_terms.append(10 * penalty_var)  # 重み10は調整可能
    ...
    if total_work_hours:
        for s in range(num_staff):
            ...
            if target is not None:
                ...
                objective_terms.append(weight * diff_var)
        ...
        for s in range(num_staff):
            ...
            if total_work_hours:
                ...
                if target is not None:
                    ...
                    objective_terms.append(-alpha * proxy)
    ...
    if support_staff_idx >= 0:
        ...
        model.Minimize(total_support_assign + sum(diff_vars) + sum(objective_terms))
    else:
            ...
            if name in leave_requests and date in leave_requests[name].get("有給", []):
                ...
                for sh in range(num_shifts):
                    if solver.Value(x[s, d, sh]) == 1:
                        result[name][date] = shift_labels[sh]
