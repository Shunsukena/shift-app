...
def to_colored_excel(df, year=None):
    ...
    for i, shift in enumerate(shifts_to_color):
        rgb = tuple(int(255 * x) for x in cmap(i % 20)[:3])  # 20色の循環
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # データ部分の色付け
        for row in range(2, len(df) + 2):  # 2行目からデータ開始
            for col in range(2, len(df.columns) + 2):  # 1列目はインデックス列
        ...
        for col_idx, col_name in enumerate(df.columns[:-1], start=2):
            try:
                # カラム名が"MM/DD"形式なら日付化して判定
                month, day = map(int, col_name.split("/"))
                date = datetime.date(year, month, day)
                if date.weekday() >= 5 or jpholiday.is_holiday(date):  # 土日祝
                    cell = ws.cell(row=1, column=col_idx)
                    cell.fill = PatternFill(start_color="ADD8E6", end_color="ADD8E6", fill_type="solid")  # 水色
            except Exception:
                # 変換失敗はスルー（例：総勤務時間列など）
                pass
